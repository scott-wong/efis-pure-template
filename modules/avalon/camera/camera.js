...
require('avalon');
var template = __inline('camera.html');
...
var widget = avalon.ui.camera = function (element, data, vmodels) {
        ...
        for (var i = 0; i < vmodel.slicedRows; i++) {
            for (var j = 0; j < vmodel.slicedCols; j++) {
                ...
                $(".oni-camera-fakehover .fakepart:last").css({
                    "background": "url(" + (vmodel.useLink ? vmodel.pictures[0].img : vmodel.pictures[0]) + ")",
        ...
        _animate = function (index, effect, indexDirection) {
            ...
            for (var i = 0; i < vmodel.slicedRows; i++) {
                for (var j = 0; j < vmodel.slicedCols; j++) {
                    ...
                    _part.css({
                        "background": "url(" + (vmodel.useLink ? vmodel.pictures[index].img : vmodel.pictures[index]) + ")",
                        "background-position": -j * fakepartWidth + "px " + -i * fakepartHeight + "px",
    ...
    if (avalon.css(element, "height") === 0) { //未设置widget高度
        _getImageSize(vmodel.useLink ? vmodel.pictures[0].img
            : vmodel.pictures[0], function () { //以第一张图片的尺寸为widget尺寸
                if (!vmodel.adaptiveWidth) {// modified by scott
                    vmodel.pictureWidth = this.width
                }
                if (!vmodel.adaptiveHeight) {// modified by scott
...
widget.defaults = {
    ...
    useLink: false,//@config  是否有链接 added by scott
