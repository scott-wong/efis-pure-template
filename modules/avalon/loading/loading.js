...
require('avalon')
var template = __inline('loading.html')
var ballTemplate = __inline('loading.bar.html')
...
addType("spin", {
        ...
        if (vmodel.svgSupport) {
            var len = vmodel.$circleData.length,
                ele = avalon(ele.getElementsByTagName("path")[0])
...
addType("spokes", {
        ...
        var count = vmodel.count,
            w = vmodel.width,
            sw = vmodel.spokesWidth,
            sh = vmodel.spokesHeight,
            index = 0,
            interval = vmodel.interval;
        if (vmodel.svgSupport) {
...
addType("spinning-bubbles", avalon.mix({}, _config["ball"], {
        ...
        var drawer = _config["ball"].drawer(vmodel),
            count = vmodel.count
        if (count >= 7) {
        ...
        var r = (vmodel.width - vmodel.widthInner) / 2,
            count = vmodel.count
...
addType("spinning-spin", avalon.mix({}, _config["spin"], {
        ...
        var ct = 360 / vmodel.padding * 3,
            r = vmodel.width / 2,
            dt = circleValueList(r, r - vmodel.widthInner / 2, ct),
            count = vmodel.count,
            interval = vmodel.interval,
            step = 360 / count
        vmodel.radius = vmodel.width / 2 - vmodel.widthInner / 2
